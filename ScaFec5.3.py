...
## V. 4.3.1 del 12-08-2025  - CLI robusta (cfstudio opzionale), default BASE_DIR=CWD, VERIFY=True, RETRIES=10; SHA-256 + retry + log
...
import base64
import platform
import hashlib as _hashlib_int
try:
    from asn1crypto import cms as _cms_int
except Exception:
    _cms_int = None
try:
    import win32crypt as _win32crypt_int  # only on Windows if available
except Exception:
...
def _robust_preprocess_base64(file_path: str):
    """
    If the file contains base64 text of a p7m, decode to a temp .p7m file and return its path.
    Returns (path, is_temp: bool)
    """
    try:
...
def _extract_with_asn1(p7m_path: str):
    ...
    try:
        ...
        if not sd:
            return None
        eci = sd.get('encap_content_info')
        if not eci:
            return None
        content = eci.get('content')
        if not content:
            return None
        econtent = content.native
        if isinstance(econtent, (bytes, bytearray)):
            try:
                return econtent.decode('utf-8')
            except UnicodeDecodeError:
                return econtent.decode('utf-8', errors='ignore')
    except Exception:
        return None
    return None

def _extract_with_winapi(p7m_path: str):
    if platform.system() != "Windows" or _win32crypt_int is None:
        return None
    try:
        with open(p7m_path, 'rb') as f:
            p7m_data = f.read()
        decoded_tuple = _win32crypt_int.CryptDecodeMessage(
            ...
            None,
            _win32crypt_int.CMSG_SIGNED,
            p7m_data,
            len(p7m_data)
        )
        if decoded_tuple and decoded_tuple[0]:
            try:
                return decoded_tuple[0].decode('utf-8')
            except UnicodeDecodeError:
                return decoded_tuple[0].decode('utf-8', errors='ignore')
    except Exception:
        return None
    return None
...
def _extract_with_openssl(p7m_path: str):
    for fmt in ('DER','PEM'):
        try:
            import subprocess
            cmd = ['openssl','cms','-verify','-noverify','-inform',fmt,'-in',p7m_path]
...
def _extract_bruteforce(p7m_path: str):
    try:
        ...
        for enc in ('utf-8','latin-1','cp1252'):
            ...
            m = _re_int.search(r'(<\?xml.*?</.*?:?FatturaElettronica>)', s, _re_int.DOTALL|_re_int.IGNORECASE)
            if m:
                return m.group(1)
            start = s.find('<?xml')
            if start != -1:
                ...
                for e in endings:
                    j = s.find(e, start)
                    if j != -1:
                        return s[start:j+len(e)]
    except Exception:
        return None
    return None

def _decode_single_p7m_to_xml(src_path: str) -> str:
    """Returns XML content string if decoded; otherwise None.
    Tries: ASN1 -> WinAPI -> OpenSSL -> Bruteforce, with base64 unwrap pre-step."""
...
def decrypt_p7m_files(input_dir, output_dir):
    """
    Migliorata: decripta/estrae i file .p7m con fallback multipli e salva XML in output_dir.
    Copia **tutti** gli .xml dal raw in output_dir (incl. *_metadato.xml).
    """
    ...
    for filename in entries:
        ...
        if lname.endswith(".p7m"):
            base, _ = os.path.splitext(filename)  # rimuove .p7m (mantiene .xml se presente prima)
            dest = os.path.join(output_dir, base)
            try:
                xml = _decode_single_p7m_to_xml(src)
                if xml:
                    # assicurati che il file di destinazione finisca con .xml se non già presente
                    if not dest.lower().endswith(".xml"):
                        dest += ".xml"
                    with open(dest, "w", encoding="utf-8") as f:
                        f.write(xml)
                    print(f"Decodifica riuscita: {filename} -> {os.path.basename(dest)}")
                else:
            ...
            # Copia anche i metadati XML e gli XML già pronti
            try:
                shutil.copy(src, os.path.join(output_dir, filename))
            except Exception as e:
                print(f"Errore copia XML {filename}: {e}")
