#!/usr/bin/env python3
"""
SISTEMA INTEGRATO FATTURE ELETTRONICHE AdE - VERSIONE 12.01
...
# CONFIGURAZIONE
SCRIPT_VERSION = "12.01"
SCRIPT_DATE = "14-08-2025"
...
class SistemaFattureV12:
    ...
    def print_banner(self):
        ...
        print(f"\n╔{'═'*78}╗")
        print(f"║{'SISTEMA FATTURE ADE v12.01':^78}║")
    ...
    def load_config(self, config_file):
        ...
        try:
            ...
            config['directory_sistema'].setdefault('struttura_id_sdi', 'con_id_sdi')
    ...
    def check_file_exists(self, id_fattura: str, temp_dir: Path) -> bool:
        ...
        try:
            ...
            if struttura_file in ['ENTRAMBE', 'ID_SDI']:
                dir_id_sdi = temp_dir / self.config['directory_sistema'].get('struttura_id_sdi', 'con_id_sdi')
                # Cerca file con pattern ID_SDI
    ...
    def download_fattura_completa(self, id_fattura: str, temp_dir: Path, tipo_doc: str) -> bool:
        ...
        CON CONTROLLO FILE ESISTENTI MIGLIORATO
        """
        try:
            # CONTROLLO FILE ESISTENTI PRIMA DEL DOWNLOAD
            ...
            # Crea sottocartelle in base alla configurazione
            dirs_to_create = []
            if struttura_file in ['ENTRAMBE', 'ORIGINALE']:
                dir_originali = temp_dir / self.config['directory_sistema'].get('struttura_originali', 'originali')
            ...
            self.logger.info(f"Scaricato completato: {id_fattura} -> {nome_originale_fattura}")
            return True
    ...
    def decode_p7m_duplicato(self, source_dir: Path):
        ...
        try:
            ...
            if struttura_file in ['ENTRAMBE', 'ID_SDI']:
                dir_id_sdi = source_dir / self.config['directory_sistema'].get('struttura_id_sdi', 'con_id_sdi')
    ...
    def organize_files(self, source_dir: Path, client_data: Dict):
        ...
        e estrazione anno dalla data documento
        """
        try:
                ...
                for file_path in source_subdir.glob('*'):
                    ...
                    try:
                        ...
                        if '_FATTURA' in file_path.name or '_METADATO' in file_path.name:
                            # File con nomenclatura ID_SDI
                            id_sdi = file_path.name.split('_')[0]
                        else:
                            # Cerca ID_SDI nel nome file originale
                            match = re.search(r'[A-Z]{2}\d{11}_[\w\d]+', file_path.stem)
                            if match:
                                id_sdi = match.group()
                            else:
                                # Cerca pattern ID_SDI semplice nel nome
                                for key in mapping_fatture.keys():
                        ...
                        if id_sdi and id_sdi in mapping_fatture:
                            ...
                            self.logger.warning(f"ID_SDI non trovato nel mapping per {file_path.name}, uso default RICEVUTE")
                        ...
                        # Per i metadati, usa l'anno della fattura associata
                            ...
                            if fattura_name.endswith('.xml'):
                                fattura_name = fattura_name[:-4]  # Rimuovi .xml
            ...
            for direzione in ['EMESSE', 'RICEVUTE']:
                ...
                if dir_path.exists():
                    for anno_dir in dir_path.glob('*'):
                        if anno_dir.is_dir():
                            file_count = sum(1 for _ in anno_dir.rglob('*') if _.is_file())
                            if file_count > 0:
                                stats[direzione][anno_dir.name] = file_count
            
            if stats:
                print("\n📊 Riepilogo per tipo e anno:")
                for direzione, anni in stats.items():
                    print(f"   {direzione}:")
                    for anno in sorted(anni.keys()):
                        print(f"      {anno}: {anni[anno]} file")
        ...
        for json_path in liste_json:
            try:
                ...
                for fattura in fatture:
                    ...
                    # Controllo migliorato file esistenti
                    if self.check_file_exists(id_fattura, temp_dir):
                        self.download_stats.add_gia_presente(tipo_doc, id_fattura)
                        skipped_count += 1
                    else:
